...
from mmrotate.utils.test import single_gpu_test
...
def parse_args():
    ...
    parser.add_argument('--name', help='file name') # add
    parser.add_argument(
        '--work-dir',
    ...
    parser.add_argument(
        '--target-class', 
        default=None,
        help='1. 리스트에 class 숫자로 입력 \n2. --show-dir과 함께 입력\nex1. --target-class [1, 2, 3]\nex2. --target-class [1]\nex3. --target-class all -> 전체 class별 저장')
    parser.add_argument(
        '--show-score-thr',
        type=float,
        default=0.3,
...
def main():
    ...
    if args.target_class is not None:
        if args.target_class == "all":
            target_class = [i for i in range(len(dataset.CLASSES))]
        else:
            target_class = [int(i) for i in args.target_class.split(',')]

    if not distributed:
        model = build_dp(model, cfg.device, device_ids=cfg.gpu_ids)
        outputs = single_gpu_test(model, data_loader, args.show, args.show_dir, args.show_score_thr, target_class=target_class)
    else:
